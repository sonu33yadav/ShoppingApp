...
function messageUpdate($campaignId, $msisdn, $status)
{
    ...
    $query = "UPDATE quicks_report set  status='$status',updated_at=now() where  msisdn='$msisdn' and quick_message_id='$campaignId' limit 1";
    $log->LogInfo("$query");
...
function smsReportUpdate($messageId, $msisdn, $status)
{
    global  $log;
    $link = connectDB();
    $campaignId = mysqli_real_escape_string($link, $campaignId);
    $query = "UPDATE messages_report set  status='$status',updated_at=now() where  msisdn='$msisdn' and message_id='$messageId' limit 1";
    $log->LogInfo("$query");
    mysqli_query($link, $query);
    mysqli_close($link);
    return true;
...
function campaignDelivered($campaignId)
{
    ...
    $link = connectDB();
    $campaignId = mysqli_real_escape_string($link, $campaignId);
    $query = "UPDATE quicks set  success=success+1 where  id='$campaignId' limit 1";
    $log->LogInfo("$query");
    mysqli_query($link, $query);
    mysqli_close($link);
    return true;
